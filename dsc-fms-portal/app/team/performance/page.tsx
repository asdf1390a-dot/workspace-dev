...
const mockPerformanceData: PerformanceMetric[] = [
  ...
  },
  {
    ...
    technical_competency: 4.7,
    task_achievement: 4.6,
    communication: 4.8,
    learning_speed: 4.5,
    reliability: 4.7,
    completion_rate: 92,
  ...
  },
  {
    ...
    technical_competency: 4.2,
    task_achievement: 4.4,
    communication: 4.1,
    learning_speed: 4.3,
    reliability: 4.5,
    completion_rate: 88,
...
export default function PerformancePage() {
  ...
  const [sortBy, setSortBy] = useState<'overall' | 'technical' | 'achievement'>('overall');
  ...
  const sortedData = [...filteredData].sort((a, b) => {
    switch (sortBy) {
      case 'technical':
        return b.technical_competency - a.technical_competency;
      case 'achievement':
        return b.task_achievement - a.task_achievement;
      default:
    ...
    <div className="space-y-6">
      ...
      <div className="flex gap-4 items-center">
        <div className="flex items-center gap-2">
          ...
          >
            ...
            <option value="technical">Sort by Technical Competency</option>
            <option value="achievement">Sort by Task Achievement</option>
          </select>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        {sortedData.map((metric) => (
          <Link key={metric.memberId} href={`/team/members/${metric.memberId}`}>
            <div className="bg-white rounded-lg shadow hover:shadow-md transition-shadow p-6 cursor-pointer h-full">
              <div className="flex items-start justify-between mb-4">
                <div>
                  <h3 className="text-lg font-semibold text-gray-900">{metric.memberName}</h3>
                  <p className="text-sm text-gray-600">Overall Score</p>
                </div>
                <div className="text-right">
                  <div className="text-3xl font-bold text-blue-600">
                    {metric.overall_score.toFixed(2)}
                  </div>
