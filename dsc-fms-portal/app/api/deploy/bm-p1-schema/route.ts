// BM-P1 Phase 1 Schema Deployment
// Direct SQL execution via service role key
...
interface DeployResult {
  ...
  details: {
    ...
    statementsSuccessful: number;
    errors: string[];
  };
}

export async function GET(): Promise<NextResponse<DeployResult>> {
  try {
    ...
    console.log(`[BM-P1 Deploy] Starting schema deployment, ${statements.length} statements`);

    let successful = 0;
    const errors: string[] = [];

    // Note: Supabase via @supabase/supabase-js doesn't support raw SQL execution
    // This API would need to use direct PostgreSQL connection
    // For now, returning error with instructions

    throw new Error(
      'Supabase client does not support raw SQL DDL execution. ' +
    ...
    console.error('[BM-P1 Deploy] Error:', errorMsg);

    return NextResponse.json(
      {
        status: 'error',
        message: '❌ Automated deployment not possible via Supabase JS client. Use manual deployment (2 min).',
        details: {
