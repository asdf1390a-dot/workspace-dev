// Travel Checklist — Create/manage checklist items
...
export async function POST(
  ...
  try {
    ...
    if (!title) {
      return NextResponse.json({ success: false, error: { code: 'VALIDATION_ERROR', message: 'Title required' } }, { status: 400 });
    }
    ...
    return NextResponse.json({ success: true, data, message: 'Checklist item created' }, { status: 201 });
  } catch (err) {
