...
-- db/42b - Phase 2: team_structure, capability_scores, portfolio_items, activity_log (db/42a 이후 실행)
...
---
-- Table 2: Capability Scores (역량 평가)
---
...
CREATE INDEX IF NOT EXISTS idx_capability_scores_member ON capability_scores(member_id);
CREATE INDEX IF NOT EXISTS idx_capability_scores_date ON capability_scores(recorded_date);
CREATE INDEX IF NOT EXISTS idx_capability_scores_member_date ON capability_scores(member_id, recorded_date DESC);

ALTER TABLE capability_scores ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Public read access" ON capability_scores;

CREATE POLICY "Public read capability scores" ON capability_scores
  FOR SELECT USING (true);

CREATE POLICY "Authenticated insert capability scores" ON capability_scores
  FOR INSERT WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated update capability scores" ON capability_scores
  FOR UPDATE WITH CHECK (auth.role() = 'authenticated');

CREATE POLICY "Authenticated delete capability scores" ON capability_scores
  FOR DELETE USING (auth.role() = 'authenticated');

---
-- Table 3: Portfolio Items (포트폴리오)
---
...
SELECT tablename FROM pg_tables WHERE tablename IN ('team_structure', 'capability_scores', 'portfolio_items', 'activity_log');
