...
################################################################################
# Phase 2A: Message Collection API — Deployment Script
#
# Starts the Phase 2A message collection service on port 3009
# Validates health check and logs deployment status
#
# Usage:
#   bash phase2a-deploy.sh          # Start service
#   bash phase2a-deploy.sh --check  # Health check only
#   bash phase2a-deploy.sh --stop   # Stop service
#
# Version: 1.0
# Created: 2026-05-30
################################################################################
...
SERVICE_NAME="Phase 2A Message Collection"
PORT="${PORT:-3009}"
...
log_msg() {
  local level="$1"
  shift
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] [$level] $*" | tee -a "$LOG_DIR/phase2a-deploy.log"
}
...
case "${1:-start}" in
  start)
    ...
    cd "$SCRIPT_DIR"
    PORT=$PORT npm start phase2a-message-collection.js >> "$LOG_DIR/phase2a-service.log" 2>&1 &
    echo $! > "$LOG_DIR/phase2a.pid"
    log_msg "INFO" "Process started with PID: $(cat "$LOG_DIR/phase2a.pid")"
  ...
  stop)
    if [ -f "$LOG_DIR/phase2a.pid" ]; then
      PID=$(cat "$LOG_DIR/phase2a.pid")
      log_msg "INFO" "Stopping $SERVICE_NAME (PID: $PID)"
      kill $PID 2>/dev/null || true
      rm -f "$LOG_DIR/phase2a.pid"
